...
func (segment *segment) isDiagonalAt45() bool {
	return Abs(segment.pointA.x-segment.pointB.x) == Abs(segment.pointA.y-segment.pointB.y)
}
...
func (diagram *diagram) addSegment(segment segment) {
	...
	if segment.isDiagonalAt45() {
		if segment.pointA.x < segment.pointB.x {
			if segment.pointA.y < segment.pointB.y {
				for i := 0; i <= segment.pointB.x-segment.pointA.x; i++ {
					diagram.matrix[segment.pointA.y+i][segment.pointA.x+i]++
				}
			} else {
				for i := 0; i <= segment.pointB.x-segment.pointA.x; i++ {
					diagram.matrix[segment.pointA.y-i][segment.pointA.x+i]++
			...
			if segment.pointA.y < segment.pointB.y {
				for i := 0; i <= segment.pointA.x-segment.pointB.x; i++ {
					diagram.matrix[segment.pointA.y+i][segment.pointA.x-i]++
				}
			} else {
				for i := 0; i <= segment.pointA.x-segment.pointB.x; i++ {
...
func readFile(input string) ([]segment, int) {
	...
	for scanner.Scan() {
		...
		if segment.isHorizontal() || segment.isVertical() || segment.isDiagonalAt45() {
